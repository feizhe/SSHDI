...
SSHDI<-function(xmat,yvec,family = "gaussian",nloop=500){
  ...
  fit1<- foreach(i = 1:nloop,.packages=c("MASS","glmnet")) %dopar% SPARE(xmat,yvec,family = family,p0 = 9)
  stopCluster(cl)
  print("done")
  INT<-sapply(1:nloop,extr,fit1=fit1,pos=1)
  BETA<-sapply(1:nloop,extr,fit1=fit1,pos=2)
  SET<-lapply(1:nloop,extr,fit1=fit1,pos=3)
  Ycount<-sapply(1:nloop,extr,fit1=fit1,pos=4)
